<template>
  <div class="container mx-auto p-4">
    <PageSeo title="全球路由追踪" description="使用RIPE Atlas网络探针从全球各地对目标主机进行路由追踪测试，分析网络路径和跃点。" />
          ...
          <div class="flex flex-col gap-4">
            ...
            <div class="flex flex-col md:flex-row gap-4">
              ...
              <div class="form-control w-full md:w-1/3">
                ...
                <select v-model="probes" class="select select-bordered w-full bg-opacity-70 bg-base-100">
              ...
              <div class="form-control w-full md:w-1/3">
                ...
                <select v-model="ipv" class="select select-bordered w-full bg-opacity-70 bg-base-100">
          ...
          <div class="mt-8">
            <Transition enter-active-class="transition-all duration-300 ease-out"
              ...
              <div v-else-if="measurementId && !resultData" class="text-center p-8">
                ...
                <button class="btn btn-primary mt-4" @click="fetchResults">
                ...
                <div class="card-body">
                  ...
                  <div class="flex flex-col md:flex-row justify-between mb-4">
                    ...
                    <div class="flex md:flex-col mt-4 md:mt-0 gap-2 md:items-end">
                      <button @click="copyShareLink" class="btn btn-sm btn-outline">
                        <i class="fas fa-link mr-1"></i>
                        {{ showCopiedMessage ? '已复制!' : '复制链接' }}
                      </button>
                      <a v-if="resultData.ripe_url" :href="resultData.ripe_url" target="_blank"
                        class="btn btn-sm btn-outline">
                        <i class="fas fa-external-link-alt mr-1"></i>
                        RIPE Atlas
                      </a>
                      <button v-if="selectedProbe && pathLines.length > 0" @click="toggleFullscreenMap"
                        class="btn btn-sm btn-outline">
                        <i class="fas fa-expand mr-1"></i>
                        全屏地图
                      </button>
                      <button v-if="selectedProbe && pathLines.length > 0" @click="clearSelectedProbe"
                        class="btn btn-sm btn-outline">
                  ...
                  <div v-if="resultData.results && resultData.results.length > 0" class="mt-4">
                    ...
                    <div class="mb-6 relative">
                      ...
                      <div id="ripe-map" class="w-full h-[400px] rounded-lg shadow-inner bg-base-200 bg-opacity-50"
                    ...
                    <div v-if="selectedProbe && selectedProbeData" class="card bg-opacity-50 bg-base-200 p-4 mb-4">
                      ...
                      <div class="overflow-x-auto">
                        <table class="table table-sm w-full">
                          ...
                          <tbody>
                            <tr v-for="(hop, index) in currentPathHops" :key="index"
                              :class="{ 'bg-success bg-opacity-5': index === 0 }">
                              <td>
                              ...
                              <td class="text-xs">
                                <template v-if="hop.geo">
                                  {{ hop.geo.country_name || countryCenters[hop.geo.country]?.country_name ||
                                    hop.geo.country || '未知' }}
                          ...
                          <tr v-for="result in processedResults" :key="result.probe_id"
                            ...
                            <td>
                              ...
                              <template v-else>
                                ...
                                {{ result.country_name || countryCenters[result.country]?.country_name || result.country
                                  || '未知' }}
                            ...
                            <td>
                              <button @click="selectedProbe = result.probe_id" class="btn btn-xs btn-outline">
  ...
  <div class="modal" :class="{ 'modal-open': showApiKeyModal }">
    <div class="modal-box">
      ...
      <div class="form-control w-full">
        ...
        <input v-model="apiKeyInput" type="text" placeholder="输入您的API KEY" class="input input-bordered w-full" />
        <label class="label">
          <span class="label-text-alt">可以在 <a href="https://ipinfo.io" target="_blank" class="link">ipinfo.io</a> 获取API
            KEY</span>
        </label>
      </div>
  ...
  <!-- 全屏地图容器 -->
  <div v-if="isFullscreenMap" class="fixed inset-0 z-40 bg-base-100">
    <div id="fullscreen-map" class="w-full h-full"></div>
    <button @click="toggleFullscreenMap"
      class="fixed right-4 top-4 z-50 btn btn-circle bg-base-200 bg-opacity-70 backdrop-blur shadow-lg" title="退出全屏">
      <i class="fas fa-times"></i>
    </button>

    <!-- 可拖动的路由跳跃表格 -->
    <div ref="routeTableWindow"
      ...
      }" @mousedown="startDrag" @mouseenter="handleMouseEnter" @mouseleave="handleMouseLeave">
      <div class="window-header p-2 flex justify-between items-center cursor-move bg-base-300 bg-opacity-80">
        ...
        <div class="flex gap-1">
          <button @click.stop="toggleTableCollapse" class="btn btn-xs btn-ghost btn-circle"
            :title="isTableCollapsed ? '展开' : '收起'" v-if="!isTableMinimized">
            <i :class="isTableCollapsed ? 'fas fa-chevron-up' : 'fas fa-chevron-down'"></i>
          </button>
          <button @click.stop="isTableMinimized = !isTableMinimized" class="btn btn-xs btn-ghost btn-circle"
